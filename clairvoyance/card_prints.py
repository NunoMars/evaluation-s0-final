...
def polarity_calcul(list_of_polarity):
    ...
    if percentage_positif == 0 or percentage_negatif == 0 or percentage_neutral == 0:
        if list_of_polarity[0] == "Positif":
            msg = ["Résultat plutôt positif avec " +
            str(percentage_positif) + "% des cartes!!"]
        if list_of_polarity[0] == "Negatif":
            msg = [" Tirage négatif avec " + str(percentage_negatif) +
...
def create_final_response(list_of_cards, name, list_of_polarity, chosed_card_deck):
    ...
    final_tittle = {"final_response_tittle" : "<div class='col'><div class='cta-inner text-center rounded'>" +
            "<h4>" + name.capitalize() + " vois-ci votre réponse!" + "</h4>" +
            "<h4>" + polarity + "</h4>" +
...
def clairvoyante_sort_cards(name, chosed_card_deck, chosed_theme):
    ...
    for card in chosed_card_deck:
        ...
        final = create_final_response(list_of_cards, name, list_of_polarity, chosed_card_deck)
